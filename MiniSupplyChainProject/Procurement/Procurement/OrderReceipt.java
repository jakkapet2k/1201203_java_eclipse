...
public class OrderReceipt extends JDialog implements ActionListener{
	...
	public void actionPerformed(ActionEvent evt) {
		...
		if(actionCommand.equals(orderString)) {
			...
			try {
				...
				Order DBorder = new Order(id, supplier, goods, quantity, orderdate);
	...
	private void addsOrder(Order order) {
		...
		try {
			String insertSql = "INSERT INTO order_goods VALUES" + "(" + order.id + ", '" + order.supplier + "', '" + order.goods + "', " + order.quantity + ", '"  + order.orderdate + "'"+",'0')";
