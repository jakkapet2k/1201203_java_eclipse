...
public class CheckOrder extends JDialog implements ActionListener {
	...
	public CheckOrder(JFrame frame) {
		...
		goodsTxt.setBounds(164, 58, 194, 20);
	...
	public void actionPerformed(ActionEvent evt) {
		...
		if (actionCommand.equals(AcceptString)) {
			...
			try {
				...
				Check Check = new Check(goods, receivedate);
	...
	private void CheckGoods(Check Check) {
		...
		try {
			String updateSql = "UPDATE order_goods SET receive_date='" + Check.receivedate  + "' WHERE goods= '" + Check.goods+"'";
			String updateGoodsSql = "UPDATE goods,order_goods SET goods.stock = goods.stock + order_goods.quantity  WHERE goods.name = order_goods.goods AND order_goods.goods = '" + Check.goods+"'";
